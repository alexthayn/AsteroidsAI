...
Bullet::Bullet(Point position, Point delta) :
	Object(Sprite(Vector(.02, .02), "img/asteroid.png", Rect(Point(.667, .333), Point(.777, .667))), position, delta, delta.dir(), 0),
	timeToLive(.5) {}
...
Ship::Ship(int type, bool player1) : isPlayer1(player1) {
	...
	sprite.size = Point(.15 / 3.0, .2 / 3.0);
...
void Ship::Update(double dt) {
	...
	deltaHeading *= 0;
	if (leftCommand && deltaHeading < turn_rate)
...
Astroid::Astroid(Point position, Point delta, double size) :
	Object(Sprite(Vector(size, size), "img/asteroid.png", Rect()), position, delta, delta.dir(), .001),
	size(size) {}
...
std::string SceneManager::GetGameState(bool isPlayer1) {
	...
	doc.SetObject();
	if (isPlayer1) {
		doc.AddMember("myShip", Player1.encode(doc.GetAllocator()), doc.GetAllocator());
		doc.AddMember("enemy", Player2.encode(doc.GetAllocator()), doc.GetAllocator());
	}
	else {
		doc.AddMember("myShip", Player2.encode(doc.GetAllocator()), doc.GetAllocator());
		doc.AddMember("enemy", Player1.encode(doc.GetAllocator()), doc.GetAllocator());
	}
